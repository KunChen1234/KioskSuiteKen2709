...
async function writeTag(comPort: SerialPort, input: string, dataParser: ReadlineParser): Promise<void> {
	...
	*/
	...
	// console.log(comms);
	await command(comPort, "041007\r", dataParser); // Initialise LEDs
	await command(comPort, "041101\r", dataParser); // Turn Red LED On
			...
			if (scanResult) {
				...
				comms.forEach(async (comm) => {
					...
					if (!writeResult || !Buffer.isBuffer(writeResult) || writeResult.subarray(0, 4).toString() !== "0001") {
						...
						// await command(comPort, "041207\r", dataParser); // Turn all LEDs Off
						// await command(comPort, "041101\r", dataParser); // Turn Red LED On
						await command(comPort, "041201\r", dataParser); // Turn Red LED Off
	...
	*/
	...
	while (runLoop) {
		...
		try {
			...
			if (scanResult) {
				// console.log(scanResult);
				await command(comPort, "041107\r", dataParser); // Turn all LEDs on
				comms.forEach(async (comm) => {
					const writeResult = await command(comPort, comm + "\r", dataParser);
					// console.log(writeResult);
					if (!writeResult || !Buffer.isBuffer(writeResult) || writeResult.subarray(0, 4).toString() !== "0001") {
 ...
 * @param {Boolean?} parser True if a dataparser should be returned as well.
 * @returns {[SerialPort, ReadlineParser | null]} The SerialPort object interface with the NFC scanner and an optional dataparser
 */
async function openComPort(port?: string, parser = true): Promise<[SerialPort, ReadlineParser | null]> {
	if (port) {
		...
		// console.log(ports);
		const nfcPortInfo = nfcPorts.filter((port) => port.vendorId === "09D8");
		if (nfcPortInfo.length === 0) {
			throw new Error("Cannot Find TWN4 NFC Scanner.")
		} else if (nfcPortInfo.length > 1) {
			console.warn("Multiple potential NFC Scanners detected. Selecting the first one");
		}
		const comPort = new SerialPort({ path: nfcPortInfo[0].path, baudRate: 9600 }, (err: Error | null | undefined) => {
			if (err) {
				console.error(err.message);
				// console.log("Unable to open serial port, exiting");
				process.exit(1);
...
async function readTag(comPort: SerialPort, dataParser: ReadlineParser, beep = true): Promise<string | null> {
	...
	await command(comPort, "041007\r", dataParser); // Initialise all LEDs
	await command(comPort, "041101\r", dataParser); // Turn Red LED On
	// eslint-disable-next-line no-constant-condition
	while (true) {
		try {
...
async function closeComPort(comPort: SerialPort, dataParser: ReadlineParser) {
	await command(comPort, "041207\r", dataParser);
	comPort.close()
}
...
async function main() {
	...
	const [comPort, dataParser] = nfcPort ? await openComPort(nfcPort) : await openComPort();
	if (!dataParser) {
		return;
	}
	await command(comPort, "041007\r", dataParser); // Initialise all LEDs
	await command(comPort, "041102\r", dataParser); // Turn Green LED On
	if (args["r"]) {
		// console.log("Reading Tags");
		console.log(await readTag(comPort, dataParser));
	}
